      ...
      {steps.map((s, i) => (
        <div key={i} className="flex gap-3">
          <div className="flex flex-col items-center">
            {s.status === 'completed' && <CheckCircle2 className="h-5 w-5 text-success" />}
            ...
            {i < steps.length - 1 && <div className={cn('w-0.5 flex-1 my-1', s.status === 'completed' ? 'bg-success' : 'bg-muted')} />}
          </div>
          <div className="pb-4">
