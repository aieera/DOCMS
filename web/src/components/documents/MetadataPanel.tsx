...
export function MetadataPanel({ doc, open, onClose }: Props) {
  return (
    <Sheet open={open} onOpenChange={onClose} title={doc.title}>
      <Tabs defaultValue="info">
        ...
        <TabsContent value="info">
          <div className="space-y-4">
            ...
            <div>
              <p className="mb-1.5 text-xs font-medium text-[var(--color-text-secondary)]">Tags</p>
              <TagEditor documentId={doc.id} tags={doc.tags} />
            </div>
          </div>
        </TabsContent>
        <TabsContent value="versions"><VersionHistory documentId={doc.id} /></TabsContent>
        <TabsContent value="activity"><p className="text-sm text-[var(--color-text-secondary)]">Activity log coming soon</p></TabsContent>
      </Tabs>
    </Sheet>
  )
}
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return <div className="flex items-center justify-between"><span className="text-[var(--color-text-secondary)]">{label}</span><span>{children}</span></div>
}
