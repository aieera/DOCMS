...
Audience: a VaultDMS user installing the "VaultDMS for Word" ribbon
...
- A VaultDMS user account that matches the email on your Microsoft
  account. The add-in looks up VaultDMS users by exact email
  match — your VaultDMS admin invited you with the same address
...
- The hosted manifest URL (your VaultDMS admin gives you this;
...
After a few seconds, a new **VaultDMS** tab appears in the ribbon
with two buttons: **Open from VaultDMS** and **Save back to VaultDMS**.
...
The first time you click **Open from VaultDMS** or **Save back to
VaultDMS** after install, Microsoft shows a consent screen listing
...
## 6. Open a VaultDMS document

1. Open Word (a blank doc or an existing one — either works).
2. Click the **VaultDMS** tab in the ribbon → **Open from VaultDMS**.
3. The task pane lists your recent VaultDMS documents.
4. Type to search; press Enter or click Search.
5. Click any result. Word opens the document in a new window.

What's happening under the hood:
- The add-in asks the VaultDMS API for the latest version's
...
2. When done, click the **VaultDMS** tab → **Save back to VaultDMS**.
...
| "no VaultDMS user with this email" (404)          | The VaultDMS admin hasn't invited your Microsoft email. Ask them to invite you.                                   |
| "email belongs to multiple VaultDMS tenants" (409)| Same email exists in 2+ tenants. The taskpane prompts which one to use; pick one.                                 |
...
Removing the add-in does NOT delete any documents in VaultDMS —
the documents stay with all their version history. Sign into
VaultDMS to delete normally if you need to.
