...
"Save to VaultDMS" add-in into their own Outlook. Use this for
...
- A VaultDMS user account that matches the email on your work /
  school Microsoft account. The add-in's SSO step looks up
  VaultDMS users by exact email match — your VaultDMS admin
...
The "Save to VaultDMS" button appears in the ribbon when you open
any email.
...
the ribbon, in a "VaultDMS" group.
...
The first time you click "Save to VaultDMS" after install, Outlook
...
2. Click **Save to VaultDMS** in the ribbon.
3. Pick a workspace + folder.
4. Optionally add tags. Leave "Include attachments" on (default).
5. Click **Save email**.

You'll get a "Saved to VaultDMS" confirmation in the taskpane.
...
| "no VaultDMS user with this email" (404)            | The VaultDMS admin hasn't invited your Microsoft email address yet. Ask them to invite you.                   |
...
| "Workspace dropdown empty"                          | Your VaultDMS account doesn't have any workspaces yet, or the session token didn't get attached on the request — refresh the taskpane (close + reopen the email). |
...
already been saved to VaultDMS. To remove those, sign into
VaultDMS and delete normally.
