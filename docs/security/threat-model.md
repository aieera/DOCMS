...
| **Lateral call to `/internal/*` from a compromised in-cluster peer.** Pre-ADR-0031 the sweep / worker endpoints authenticated via the shared `VAULTDMS_GATEWAY_SECRET`; any Go service holding it could invoke `POST /internal/v1/acknowledgement/sweep-reminders` with an arbitrary `X-Auth-Tenant-ID`. | Elevation-of-privilege (tenant-bound side-effects: `reminded_count`, `escalated_at`, hash-chain events). | `/internal/*` now goes through `pkg/internalauth` (ADR 0031). mTLS mode validates the peer cert against the internal CA and requires the cert's DNS SAN to appear in a per-service allowlist — possessing the gateway secret alone is no longer sufficient. HMAC mode is dual-mounted during rollout so services can cut over one-by-one, and binds `method ‖ path ‖ timestamp ‖ body` with a 5-minute skew window (replay-proof). A bad peer cert in ModeBoth fails closed — it does not fall through to HMAC. | Low — attacker needs an internal-CA-issued cert with the allowlisted SAN OR the internal HMAC secret distinct from the gateway secret. |
| **Geofence / rate-limit bypass via spoofed `X-Forwarded-For`.** Pre-ADR-0031, four middlewares (geofence, request log, rate limit, ack handler) read XFF with inconsistent and attacker-friendly logic (leftmost hop for rate limit; rightmost hop unconditionally for geofence). A client could rotate or pin XFF values to evade per-IP controls. | Tampering of source-IP attestation → DoS / geofence bypass. | `pkg/trustedproxy` is the single source of truth. It walks XFF right-to-left and only believes a hop whose immediate predecessor is in `VAULTDMS_TRUSTED_PROXY_CIDRS`; the first untrusted hop is the real client. `VAULTDMS_ENV=production` with an empty CIDR list panics at boot — a misconfigured deploy fails loudly instead of silently trusting spoofed headers. | Low — attacker needs to compromise or MITM a trusted-CIDR proxy (gateway) to inject an XFF value the middleware will honour. |
