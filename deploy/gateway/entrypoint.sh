...
# is a decK-time feature), so the `${SEDOC_GATEWAY_SECRET}` token in
...
: "${SEDOC_GATEWAY_SECRET:?SEDOC_GATEWAY_SECRET must be set}"
...
sed "s|\${SEDOC_GATEWAY_SECRET}|${SEDOC_GATEWAY_SECRET}|g" "$SRC" > "$OUT"
